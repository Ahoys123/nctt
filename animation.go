...
func NewSlowText(text string, bound *Rect, w *TermWindow) *SlowText {
	return &SlowText{[]rune(text), bound, 0, w, false, 0, nil, nil, false}
}

func NewTypewritter(text string, bound *Rect, w *TermWindow, click *SoundEffect, ding *SoundEffect) *SlowText {
...
func NewPopUp(text string, width, height int, bound *Rect, w *TermWindow) *PopUp {
...
func (pu *PopUp) Update(ec []event) {
	...
	if pu.showNext {
		...
		FillRect(' ', pu.hovBox, w)
		DrawBoxAround(pu.hovBox, popupBox, w)
		DrawText(pu.text, pu.hovBox, popupBox, w)
		pu.showNext = false
	}
...
func (pu *PopUp) Reset() {
	if pu.hovBox != nil {
		FillRect(' ', &Rect{pu.hovBox.x - 1, pu.hovBox.y - 1, pu.hovBox.w + 2, pu.hovBox.h + 2}, w)
		pu.hovBox = nil
		pu.repContent = nil
	}
...
func (dc *drawCall) Draw(w *TermWindow) {
	DrawTextOffset(dc.text, dc.rect, dc.offset, dc.style, w)
}

func NewHoverText(text string, r *Rect, dict Replacer, w *TermWindow) *HoverText {
...
func NewOptions(options []string, r *Rect, w *TermWindow) *Options {
	drawCalls := []drawCall{}
	row := r.y
...
func (ti *TextInput) Update(ec []event) {
	...
	if len(ec) > 0 { // if input
		...
		case *keyEvent: // if key
			...
			FillRect(' ', ti.uir, w)
			DrawText(string(ti.userText), ti.uir, normal, w)
			ti.curmx++
